...
notice : learning rate값이 크면 학습이 잘 안된다. 논문참조
...
private:
    ...
    float m_decay; ///< m_aaMeanSquared, m_aaMeanGrad, gradient 조정 가중치 값
    float m_epsilon; ///< 분모 값이 0이 되는 것을 방지 하는 값

    bool m_centered;   ///< cemtered version enable boolean 변수
...
public:
  ...
  */
    ...
    */
    ...
    int UpdateParameter(Operator<DTYPE> *pParameter, Tensor<DTYPE> *pMeanSquared) {
        ...
        for (int i = 0; i < capacity; i++) {
            (*pMeanSquared)[i]       = (m_decay * (*pMeanSquared)[i]) + (( 1.f - m_decay) * ((*gradient)[i] * (*gradient)[i]));
            (*trainable_data)[i]    += ((signed_learning_rate * (*gradient)[i]) / std::sqrt((*pMeanSquared)[i] + m_epsilon));
        }
    ...
    */
    ...
    int UpdateParameter(Operator<DTYPE> *pParameter, Tensor<DTYPE> *pMeanSquared, Tensor<DTYPE> *pMeanGrad){
        Tensor<DTYPE> *trainable_data = pParameter->GetResult();
        Tensor<DTYPE> *gradient       = pParameter->GetGradient();
        ...
        for (int i = 0; i < capacity; i++) {
            (*pMeanGrad)[i]     = (m_decay * (*pMeanGrad)[i]) + ((1.f - m_decay) * (*gradient)[i]);
            (*pMeanSquared)[i]  = (m_decay * (*pMeanSquared)[i]) + (( 1.f - m_decay) * ((*gradient)[i] * (*gradient)[i]));
            (*trainable_data)[i] += ((signed_learning_rate * (*gradient)[i]) / std::sqrt(((*pMeanSquared)[i] - ((*pMeanGrad)[i] * (*pMeanGrad)[i]))+ m_epsilon));
        }
    ...
    */
    ...
    @param pParameter 업데이트 할 Tensor를 가지고 있는 Operator포인터
    @param pMeanSquared 업데이트 할 pMeanSquared
    @see__global__ void RMSPropUpdate_kernel(float *pDevWeight, float *pDevAccGradient, int weightDim, float signed_learning_rate, float decay, float epsilon, float weightDecayRate, float *pMeanSquared)
    @return 성공 시 TURE
    */
    ...
    */
    ...
    int UpdateParameterOnGPU(Operator<DTYPE> *pParameter, Tensor<DTYPE> *pMeanSquared, Tensor<DTYPE> *pMeanGrad);
